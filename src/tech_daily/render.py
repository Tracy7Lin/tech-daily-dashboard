...
def _render_chat_agent_shell(context: dict) -> str:
    ...
    return (
        ...
        "<p class='chat-status' id='chat-status'>可以直接问我今天重点、主专题、某家公司最近几天的动作，或者当前信源状态。</p>"
        ...
        "  input.disabled = true;"
        "  try {"
        "    const mode = context.mode_used || 'rule';"
        "    const response = answer(question);"
        "    addMessage('agent', response);"
        "    status.textContent = mode === 'rule' ? '当前使用本地规则问答，可继续追问。' : '当前使用增强问答模式，可继续追问。';"
        "  } catch (error) {"
        "    addMessage('agent', '这次问答没有顺利完成，我先回退到日报内可直接支持的问题：今天重点、主专题、某家公司、信源状态。');"
        "    status.textContent = '问答已降级到安全模式。';"
        "  } finally {"
        "    send.disabled = false;"
