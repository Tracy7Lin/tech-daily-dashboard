...
class LLMEditorialTests(unittest.TestCase):
    def test_build_daily_headline_removes_meta_phrasing(self) -> None:
        editorial = LLMEditorial(
            _StubClient({"headline": "可以看出，今天最值得关注的是模型与平台能力开始外溢到更具体的产品入口。"})
        )
        company_reports = [type("Report", (), {"company_name": "OpenAI", "has_updates": True})()]
        topic_clusters = [type("Cluster", (), {"title": "模型与能力发布", "entries": [1]})()]
